...
#define STARTING_PAYLOAD_CHUNK_SIZE 1792
...
control MyIngress(inout headers hdr,
    ...
    apply {
        if (hdr.tcp.isValid()) {
            sha256_hash_1024(hdr.payload.sha256, hdr.payload.payload, hdr.payload.payload2, hdr.payload.payload3, hdr.payload.payload4);          
        }
        if (hdr.ipv4.isValid()) {
            ipv4_exact.apply();
        } 
